...
GeocachingApi.prototype.searchForGeocaches= function(params, cb)
{
    ...
    if (array_key_exists('PointRadiusLatitude', params) && array_key_exists('PointRadiusLongitude', params)) {
        _.merge(post_params, {
            PointRadius: {
                Point: {
    ...
    if (array_key_exists('bounds', params)) {
        var bounds = params['bounds']; //LngLatBounds
        _.merge(post_params, {
            Viewport: {
                BottomRight: {
                    Latitude: bounds[1][1],
