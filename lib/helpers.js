...
//
// Distance consists of the number plus the unit of measurement. 
// Currently supports km (kilometer), mi (miles), and m (meters). 
// Max value allowed is equivalent to 100 miles (if value passed in is greater than max, 
// radius will be set to max value)	
//
exports.parseNumberWithUnit = function(text){
  const m = text.exec(/(\d+)([kmi]+)/);
  return {
    distance: parseInt(m[0]),
    unit: m[1]
  };
}